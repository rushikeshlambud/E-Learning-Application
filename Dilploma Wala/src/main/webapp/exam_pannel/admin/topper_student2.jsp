...
<!--<%=wrong_ans.getCount(quiz_id,rs.getString(8))%>-->
...
sql="select count(ans.user_id),ans.user_id,qd.negative_marking,qd.right_ans_mark,qd.wrong_ans_mark,ud.stdnumber,ud.studentname,ud.student_id from answerkey as ans inner join question as ques on ans.question_id=ques.id inner join register as ud on ans.user_id=ud.student_id inner join quiz_detail as qd on ans.quiz_id=qd.id where ques.quiz_id=? and ans.answer!=ques.answer group by ans.user_id,ans.quiz_id";
ps=con.prepareStatement(sql);
ps.setString(1, quiz_id);
rs=ps.executeQuery();
while(rs.next()){
 int wnd=wrong_ans.getCount(quiz_id,rs.getString(8));
	if(wnd==0)
	{
    %>
<td><%=i %></td>
...
<td>
	...
	<%wnd=wrong_ans.getCount(quiz_id,rs.getString(8));
	if(wnd==0)
	{
		%>0<%
	}
