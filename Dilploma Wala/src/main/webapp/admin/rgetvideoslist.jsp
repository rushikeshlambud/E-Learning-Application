...
<link rel="stylesheet" href="../js/dataTables.bootstrap4.css">
<style>
.pp
{
...
<div class="row " style="min-height:600px;">
	<div class="col-md-2">
       ...
       <button class="btn btn-primary btn-large r1">Track-1</button><button class="btn btn-primary r2">Track-2</button>
       <br>
       <br>
       <div class="r11">
       <table id="dataTableExample" class="table table-bordered">
       			...
       			PreparedStatement ps=con.prepareStatement("select * from repe_videos where rep_id=? and subject_id=? and batch_id=? and std='11th';");
       			...
       			<tr class="bg-white t<%=i%>">
       				...
       				<td><% JSONObject obj= Subjects.getsubjectsbyId(rs.getString(9)); %>
       				<label>Subject Name : <%if(obj.length()!=0){%><%=obj.getString("sub_Name") %><%} %></label>
					<%  obj= Batch.getBatchbyId(rs.getString(11));%>
					<br>
       				...
       				<td>
       					...
       					<button title="Delete" class="btn btn-danger" onclick="rdelete_video(<%=rs.getString(1)%>,<%=i%>)"><span class="fa fa-trash"></span></button></td>
       			</tr>
       			<%}%>
       			</tbody>
       </table>
       ...
       <table class="table table-bordered">
       			...
       			<tbody>
       			<%
       		    obj1=Batch.getBatchIdbyname(request.getParameter("xsb"), request.getParameter("btc"));
       		    con=DbConnect.con();
       			ps=con.prepareStatement("select * from repe_videos where rep_id=? and subject_id=? and batch_id=? and std='12th';");
       			ps.setString(1,request.getParameter("cls"));
       			ps.setString(2,request.getParameter("xsb"));
       			ps.setString(3,obj1.getString("batch_id"));
       		    rs=ps.executeQuery();
       			...
       			<tr class="bg-white rt<%=i%>">
					...
					<br>
       				...
       				<p><%=rs.getString(10) %></p></td>
       				<td>
       					<button title="view" class="btn btn-primary"><span class="fa fa-eye vview"></span></button>
       					<button title="Update" class="btn btn-success" onclick="rupdatevideo(<%=rs.getString(1)%>,<%=i%>)"><span class="fa fa-edit"></span></button>
       					<button title="Delete" class="btn btn-danger" onclick="r1delete_video(<%=rs.getString(1)%>,<%=i%>)"><span class="fa fa-trash"></span></button></td>
       			</tr>
       			<%}con.close(); %>
       			</tbody>
       </table>
      ...
      <div class="video-pop-up">
		...
		<h3>Update Video Details</h3>
			...
			<form class="form-horizontal" id="form">
        			...
        			<div class="form-group">
        			<button class="btn btn-success" onclick="rupdate_evideo()" type="button">Submit</button>
        			</div>
        		</form>
...
<script src="../js/data-table.js"></script>
      ...
      $('.r1').click(function(){
    	 ...
    	 $('.r2').css('background-color','#2250fc');
      ...
      $('.r2').click(function(){
     	 ...
     	 $('.r1').css('background-color','#2250fc');
