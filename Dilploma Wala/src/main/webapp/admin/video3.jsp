...
<div class="row " style="min-height:600px;">
	<div class="col-md-2">
        ...
        PreparedStatement ps=con.prepareStatement("SELECT distinct b.batch_name FROM batch_name as b inner join subject as s on b.subject_id=s.subject_id where s.standard='11th'");
       ResultSet rs=ps.executeQuery();
        ...
        <div class="col-md-9 body-pannel">
			...
			<div class="col-md-4 ad-batches">
        		...
        		<%JSONArray arr=Subjects.getsubjectbyClassName("11th",rs.getString(1)); %>
        		...
        		%>
        		<a href="getvideoslist.jsp?xsb=<%=obj.getString("sub_Id")%>&cls=11th&btc=<%=rs.getString(1)%>" style="color:white"><%=obj.getString("sub_Name")%></a>
        		<%}
        		}
        		if(kj==0)
        		{%>
  ...
  <hr style="background: green; height: 2px;">
     ...
     <div class="row">
     <%
     ps=con.prepareStatement("SELECT distinct b.batch_name FROM batch_name as b inner join subject as s on b.subject_id=s.subject_id where s.standard='12th'");
     rs=ps.executeQuery();
    %>
			<%while(rs.next()){ 
				 kj=0;
			...
			<div class="col-md-4 ad-batches">
        		<div class="batch-sub">
        		<h1><%=rs.getString(1)%></h1>
        		<%JSONArray arr=Subjects.getsubjectbyClassName("12th",rs.getString(1)); %>
        		...
        		if(!obj.getString("combine").equals("yes")){
        		%>
        		<a href="getvideoslist.jsp?xsb=<%=obj.getString("sub_Id")%>&cls=12th&btc=<%=rs.getString(1)%>" style="color:white"><%=obj.getString("sub_Name")%></a>
        		<%}
        		} if(kj==0)
        		{%>
        			<a href="uploadvideo.jsp"><i class="fa fa-plus"></i> Add Videos</a>
     ...
     <div class="row">
         <%
     ps=con.prepareStatement("SELECT distinct b.batch_name FROM batch_name as b inner join subject as s on b.subject_id=s.subject_id where s.standard='Repeater'");
     rs=ps.executeQuery();
     ...
     %>	<%while(rs.next()){
    	 ...
    	 %>
			<div class="col-md-4 ad-batches">
        		<div class="batch-sub">
        		<h1><%=rs.getString(1)%></h1>
        		<%JSONArray arr=Subjects.getsubjectbyClassName("Repeater",rs.getString(1)); %>
        		...
        		if(!obj.getString("combine").equals("yes")){
        		%>
       <a href="getvideoslist.jsp?xsb=<%=obj.getString("sub_Id")%>&cls=Repeater&btc=<%=rs.getString(1)%>" style="color:white"><%=obj.getString("sub_Name")%></a>
